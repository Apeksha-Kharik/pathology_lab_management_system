...
function ReceptionistLogin() {
  ...
  const submit = async (e) => {
    ...
    try {
      const data = await loginReceptionist(form);
      localStorage.setItem("token", data.token);
      localStorage.setItem("user", JSON.stringify(data.user));
      localStorage.setItem("userId", data.user.id);
      window.location.href = data.user.mustChangePassword ? "/change-password" : "/receptionist/dashboard";
    } catch (error) {
      alert(error.response?.data?.message || "Receptionist login failed");
    }
  };
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-100 p-6">
      <form onSubmit={submit} className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <h1 className="mb-2 text-2xl font-bold text-slate-900">Receptionist Login</h1>
        <p className="mb-6 text-sm text-slate-500">Login with your staff account.</p>
